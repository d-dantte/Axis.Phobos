namespace Axis.Phobos.Extensions
{
    ...
    public static class DelegateAdapterExtensions
    {
        ...
        where CallbackDelegate : class
        {
            ...
            var toRemove = (source as Delegate).GetInvocationList()
                                               .Where(d => d.TrueTarget() is DelegateAdapter<CallbackDelegate>)
                                               .Select(d => new { Target = d.TrueTarget() as DelegateAdapter<CallbackDelegate>, Delegate = d })
        ...
        internal static Func<DelegateAdapter<CallbackDelegate>> MediatorFor<CallbackDelegate>()
        where CallbackDelegate : class
        {
            return _ProxyCache.GetOrAdd(typeof(CallbackDelegate), cbdt =>
            {
                #region public class __AxisDynType_DelegateAdapter[*]{...}
                var cbdSignature = cbdt.DelegateSignature();
                var cbdParams = cbdSignature.GetParameters();
                DelegateAdapter<CallbackDelegate> _this = null;
                var dmType = typeof(DelegateAdapter<CallbackDelegate>);
                var callbackMinfo = dmType.GetRuntimeMethod(nameof(_this.Callback), new[] { typeof(object[]) });
                var tbuilder = _moduleBuilder.DefineType($"{DynamicDelegateTypeNamePrefix}DelegateAdapter_{Guid.NewGuid().ToString().Replace("-", "_")}",
                ...
                Func<DelegateAdapter<CallbackDelegate>> creator = () => Activator.CreateInstance(type) as DelegateAdapter<CallbackDelegate>;
                return creator;
            })
            as Func<DelegateAdapter<CallbackDelegate>>;
        }
